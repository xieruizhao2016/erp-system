...
# 上传应用镜像到服务器并导入脚本
# 使用方法: ./upload-app-images.sh [镜像文件路径] [服务器地址]
...
if [ -z "$1" ]; then
    ...
    if [ -d "$EXPORT_DIR" ]; then
        LATEST_FILE=$(ls -t "$EXPORT_DIR"/app-images-*.tar.gz 2>/dev/null | head -1)
        if [ -n "$LATEST_FILE" ]; then
            IMAGE_FILE="$LATEST_FILE"
            echo -e "${BLUE}自动选择最新镜像文件: $IMAGE_FILE${NC}"
        else
            echo -e "${RED}错误: 未找到镜像文件${NC}"
            echo "请先执行: ./deploy/scripts/export-app-images.sh"
            exit 1
        fi
    else
...
SSH_CMD="gunzip -c $REMOTE_FILE | docker load && rm -f $REMOTE_FILE && echo '✅ 镜像导入成功' && docker images | grep -E 'yudao-server|yudao-admin'"
...
if [ $? -eq 0 ]; then
    ...
    echo "3. 启动服务: docker-compose -f docker-compose.prod.yml --env-file .env up -d"
    echo ""
else
