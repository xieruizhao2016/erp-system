<template>
  <ContentWrap>
    <DocAlert title="【库存】内部入库" url="https://doc.iocoder.cn/erp/stock-internal-in/" />
...
/** ERP 内部入库 列表 */
defineOptions({ name: 'ErpStockInternalIn' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<StockInternalInVO[]>([]) // 列表的数据
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await StockInternalInApi.deleteStockInternalIn(id)
...
const handleUpdateStatus = async (id: number, status: number) => {
  try {
    ...
    await StockInternalInApi.updateStockInternalInStatus(id, status)
...
const handleExport = async () => {
  try {
    ...
    const data = await StockInternalInApi.exportStockInternalIn(queryParams)
    download.excel(data, 'ERP 内部入库.xls')
  } catch {
  } finally {
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: StockInternalInVO[]) => {
