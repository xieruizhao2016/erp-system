    ...
    <ContentWrap>
      <el-tabs v-model="subTabsName" class="-mt-15px -mb-10px">
        <el-tab-pane label="出库产品清单" name="item">
          <StockInternalOutItemForm ref="itemFormRef" :items="formData.items" :disabled="disabled" />
        </el-tab-pane>
      </el-tabs>
    </ContentWrap>
    <template #footer>
...
import StockInternalOutItemForm from './components/StockInternalOutItemForm.vue'

/** ERP 内部出库 表单 */
defineOptions({ name: 'ErpStockInternalOutForm' })
...
const formRules = reactive({
  ...
  warehouseId: [{ required: true, message: '调出仓库不能为空', trigger: 'change' }],
  outTime: [{ required: true, message: '出库时间不能为空', trigger: 'change' }]
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await StockInternalOutApi.getStockInternalOut(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as StockInternalOutVO
    if (formType.value === 'create') {
      await StockInternalOutApi.createStockInternalOut(data)
      message.success(t('common.createSuccess'))
    } else {
      await StockInternalOutApi.updateStockInternalOut(data)
