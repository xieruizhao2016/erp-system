...
const formRules = reactive({
  ...
  amount: [{ required: true, message: '预付金额不能为空', trigger: 'blur' }]
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await PrepaymentApi.getPrepayment(id)
      // 计算余额
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as PrepaymentVO
    if (formType.value === 'create') {
      await PrepaymentApi.createPrepayment(data)
      message.success(t('common.createSuccess'))
    } else {
      await PrepaymentApi.updatePrepayment(data)
