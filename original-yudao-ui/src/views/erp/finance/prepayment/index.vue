<template>
  <ContentWrap>
    <DocAlert title="【财务】预付款" url="https://doc.iocoder.cn/erp/finance-prepayment/" />
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button
            ...
            v-hasPermi="['erp:finance-prepayment:export']"
          >
    ...
    <ContentWrap>
      <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
        ...
        <el-table-column label="预付金额" align="center" prop="amount" :formatter="erpPriceTableColumnFormatter" />
    ...
    <!-- 表单弹窗：添加/修改 -->
    <PrepaymentForm ref="formRef" @success="getList" />
  </ContentWrap>
</template>
...
import { PrepaymentApi, PrepaymentVO } from '@/api/erp/finance/prepayment'
...
const list = ref<PrepaymentVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await PrepaymentApi.getPrepaymentPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await PrepaymentApi.deletePrepayment(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await PrepaymentApi.exportPrepayment(queryParams)
    download.excel(data, 'ERP 预付款.xls')
  } catch {
  } finally {
