-- 为租户163的所有角色添加内部入库和出库权限
-- 问题：租户163的角色缺少内部入库和出库的菜单权限（type=2）
-- 解决：为租户163的所有角色添加type=2的菜单权限
...
WHERE u.tenant_id = 163
  AND NOT EXISTS (SELECT 1 FROM system_role_menu WHERE role_id = r.id AND menu_id = @internal_in_menu_1);
...
JOIN system_role r ON ur.role_id = r.id
WHERE u.tenant_id = 163
  AND NOT EXISTS (SELECT 1 FROM system_role_menu WHERE role_id = r.id AND menu_id = @internal_in_menu_2);

-- 为租户163的所有角色添加内部出库菜单权限
INSERT INTO system_role_menu (role_id, menu_id)
...
JOIN system_role r ON ur.role_id = r.id
WHERE u.tenant_id = 163
  AND NOT EXISTS (SELECT 1 FROM system_role_menu WHERE role_id = r.id AND menu_id = @internal_out_menu_1);
...
JOIN system_role r ON ur.role_id = r.id
WHERE u.tenant_id = 163
  AND NOT EXISTS (SELECT 1 FROM system_role_menu WHERE role_id = r.id AND menu_id = @internal_out_menu_2);
