...
-- 获取下一个可用的ID（从6200开始）
SET @sku_menu_id = 6200;
SET @package_menu_id = 6201;
SET @oem_menu_id = 6202;

-- ========== 1. SKU管理 ==========
INSERT INTO `system_menu` (`id`, `name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
SELECT @sku_menu_id, 'SKU管理', '', 2, 3, @product_parent_id, 'product-sku', 'ep:goods', 'erp/productsku/index', 'ErpProductSku', 0, b'1', b'1', b'1', '1', NOW(), '1', NOW(), b'0'
WHERE NOT EXISTS (SELECT 1 FROM system_menu WHERE name = 'SKU管理' AND parent_id = @product_parent_id AND deleted = 0);
...
-- ========== 2. 产品包装 ==========
INSERT INTO `system_menu` (`id`, `name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
SELECT @package_menu_id, '产品包装', '', 2, 4, @product_parent_id, 'product-package', 'ep:box', 'erp/productpackage/index', 'ErpProductPackage', 0, b'1', b'1', b'1', '1', NOW(), '1', NOW(), b'0'
WHERE NOT EXISTS (SELECT 1 FROM system_menu WHERE name = '产品包装' AND parent_id = @product_parent_id AND deleted = 0);
...
-- ========== 3. 产品OEM ==========
INSERT INTO `system_menu` (`id`, `name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `component_name`, `status`, `visible`, `keep_alive`, `always_show`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
SELECT @oem_menu_id, '产品OEM', '', 2, 5, @product_parent_id, 'product-oem', 'ep:office-building', 'erp/productoem/index', 'ErpProductOem', 0, b'1', b'1', b'1', '1', NOW(), '1', NOW(), b'0'
WHERE NOT EXISTS (SELECT 1 FROM system_menu WHERE name = '产品OEM' AND parent_id = @product_parent_id AND deleted = 0);
