# 生产管理系统开发计划 - 细化版

## 📋 项目概述

### 系统名称：生产管理系统 (Production Management System, PMS) v2.0
### 开发理念：基于现有yudao-ERP系统，构建智能化生产管理解决方案
### 开发模式：敏捷开发 + 微服务架构，持续集成部署
### 开发周期：总周期约 18-20 周，分 4 个阶段实施
### 技术栈：Spring Boot + MyBatis-Plus + Vue3 + TypeScript + Element Plus

---

## 🏗️ 现有系统基础分析

### 已有模块深度分析

**技术架构优势：**
- ✅ Yudao框架：完整的RBAC权限体系 + 多租户支持
- ✅ 代码生成器：快速生成CRUD，提升开发效率
- ✅ 工作流引擎：Activiti 7，支持复杂流程审批
- ✅ 数据库：MySQL 8.0，支持复杂查询和事务
- ✅ 前端：Vue3 + TypeScript，现代化开发体验

**现有数据表（已梳理，共28张）：**
```
销售模块（7张）：订单、出库、退货、客户
产品模块（3张）：产品、分类、单位
采购模块（7张）：订单、入库、退货、供应商
财务模块（5张）：收款、付款、账户
库存模块（6张）：库存、出入库、盘点、调拨、记录、仓库
```

**待补全功能：**
- ⚠️ 缺少BOM管理（物料清单）
- ⚠️ 缺少工艺路线管理
- ⚠️ 缺少生产排程功能
- ⚠️ 缺少质量管理体系
- ⚠️ 缺少成本核算体系

---

## 🎯 Phase 1: 生产订单管理 - 详细方案

### 📅 时间安排：4周（28天）

### 🏛️ 详细架构设计

#### 后端架构
```
src/main/java/cn/iocoder/yudao/module/erp/service/production/
├── order/
│   ├── ProductionOrderService.java              # 生产订单服务
│   ├── ProductionOrderItemService.java          # 生产订单明细服务
│   └── MaterialConversionService.java           # 物料转化服务
├── controller/
│   ├── ProductionOrderController.java           # 生产订单API
│   └── MaterialConversionController.java        # 物料转化API
└── dal/
    ├── ProductionOrderMapper.java               # 生产订单Mapper
    ├── ProductionOrderItemMapper.java           # 明细Mapper
    └── MaterialConversionMapper.java            # 转化Mapper
```

#### 前端架构
```
src/views/erp/production/order/
├── index.vue                                    # 生产订单列表
├── create.vue                                   # 创建生产订单
├── detail.vue                                   # 生产订单详情
├── components/
│   ├── ProductionOrderForm.vue                 # 生产订单表单
│   ├── MaterialConversionForm.vue              # 物料转化表单
│   └── ProductionOrderStatusFlow.vue           # 状态流转组件
└── api/
    ├── order.ts                                 # 订单API
    └── materialConversion.ts                   # 转化API
```

### 🗄️ 数据库详细设计

#### 1. erp_production_order（生产订单主表）
```sql
CREATE TABLE erp_production_order (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    no VARCHAR(50) NOT NULL COMMENT '生产订单号',
    customer_id BIGINT COMMENT '客户ID（关联销售订单）',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    product_name VARCHAR(255) NOT NULL COMMENT '产品名称',
    product_spec VARCHAR(500) COMMENT '产品规格',
    unit_id BIGINT COMMENT '单位ID',
    quantity DECIMAL(20,4) NOT NULL COMMENT '生产数量',
    completed_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '已完成数量',
    start_time DATETIME COMMENT '计划开始时间',
    end_time DATETIME COMMENT '计划完成时间',
    actual_start_time DATETIME COMMENT '实际开始时间',
    actual_end_time DATETIME COMMENT '实际完成时间',
    status INT NOT NULL COMMENT '状态：1-待开始，2-进行中，3-已暂停，4-已完成，5-已取消',
    priority INT DEFAULT 3 COMMENT '优先级：1-紧急，2-高，3-中，4-低',
    source_type INT DEFAULT 1 COMMENT '来源类型：1-手动创建，2-销售订单，3-库存补充',
    source_id BIGINT COMMENT '来源单据ID',
    description TEXT COMMENT '生产说明',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    -- 索引
    INDEX idx_tenant_no (tenant_id, no),
    INDEX idx_status (status),
    INDEX idx_customer (customer_id),
    INDEX idx_product (product_id),
    INDEX idx_create_time (create_time)
) COMMENT='生产订单';
```

#### 2. 现有表字段扩展
```sql
-- 产品表扩展
ALTER TABLE erp_product ADD COLUMN production_type INT DEFAULT 1 COMMENT '产品类型：1-原材料，2-半成品，3-成品';
ALTER TABLE erp_product ADD COLUMN is_bom TINYINT DEFAULT 0 COMMENT '是否有BOM';

-- 仓库表扩展
ALTER TABLE erp_warehouse ADD COLUMN warehouse_type INT DEFAULT 1 COMMENT '仓库类型：1-原材料仓，2-半成品仓，3-成品仓，4-废品仓';

-- 销售订单表扩展
ALTER TABLE erp_sale_order ADD COLUMN production_status INT DEFAULT 0 COMMENT '生产状态：0-无需生产，1-生产中，2-生产完成';

-- 采购订单表扩展
ALTER TABLE erp_purchase_order ADD COLUMN production_order_id BIGINT COMMENT '关联生产订单ID';

-- 库存出入库表扩展
ALTER TABLE erp_stock_in ADD COLUMN production_order_id BIGINT COMMENT '关联生产订单ID';
ALTER TABLE erp_stock_out ADD COLUMN production_order_id BIGINT COMMENT '关联生产订单ID';
```

### 🔧 核心功能实现

#### 生产订单状态流转
```java
public enum ProductionOrderStatusEnum {
    PENDING(1, "待开始"),
    IN_PROGRESS(2, "进行中"),
    PAUSED(3, "已暂停"),
    COMPLETED(4, "已完成"),
    CANCELLED(5, "已取消");

    // 状态流转规则
    private static final Map<Integer, Set<Integer>> STATUS_FLOW = Map.of(
        PENDING.getCode(), Set.of(IN_PROGRESS.getCode(), CANCELLED.getCode()),
        IN_PROGRESS.getCode(), Set.of(PAUSED.getCode(), COMPLETED.getCode(), CANCELLED.getCode()),
        PAUSED.getCode(), Set.of(IN_PROGRESS.getCode(), CANCELLED.getCode())
    );

    public static boolean canTransition(Integer fromStatus, Integer toStatus) {
        return STATUS_FLOW.getOrDefault(fromStatus, Set.of()).contains(toStatus);
    }
}
```

### 📊 Phase 1 交付清单

#### 后端交付物
- [ ] 生产订单服务层（15个核心方法）
- [ ] 物料转化服务层（10个核心方法）
- [ ] RESTful API接口（25个接口）
- [ ] 数据库表结构和初始化脚本
- [ ] 单元测试（覆盖率≥80%）
- [ ] 集成测试（5个核心场景）

#### 前端交付物
- [ ] 生产订单列表页面（支持高级搜索、状态筛选）
- [ ] 生产订单创建页面（支持从销售订单转化）
- [ ] 生产订单详情页面（状态流转、物料转化）
- [ ] 物料转化页面（投料、产出、返工、报废）
- [ ] 生产统计报表（订单完成率、物料消耗分析）

#### 文档交付物
- [ ] API接口文档（Swagger）
- [ ] 数据库设计文档
- [ ] 业务流程图
- [ ] 用户操作手册
- [ ] 部署运维文档

---

## 🎯 Phase 2: 生产计划管理 - 详细方案

### 📅 时间安排：5周（35天）

### 🎯 核心目标
构建智能生产计划体系，实现从BOM到排产的全流程管理

### 🗄️ 数据库详细设计

#### 1. BOM管理相关表
```sql
-- BOM主表
CREATE TABLE erp_product_bom (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    bom_no VARCHAR(50) NOT NULL COMMENT 'BOM编号',
    bom_name VARCHAR(255) NOT NULL COMMENT 'BOM名称',
    version VARCHAR(20) DEFAULT '1.0' COMMENT '版本号',
    effective_date DATETIME COMMENT '生效日期',
    expire_date DATETIME COMMENT '失效日期',
    bom_type INT DEFAULT 1 COMMENT 'BOM类型：1-生产BOM，2-设计BOM，3-工艺BOM',
    standard_cost DECIMAL(20,2) COMMENT '标准成本',
    total_material_cost DECIMAL(20,2) COMMENT '总材料成本',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-生效，3-失效',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_product (tenant_id, product_id),
    INDEX idx_bom_no (tenant_id, bom_no),
    INDEX idx_status (status)
) COMMENT='BOM主表';

-- BOM明细表
CREATE TABLE erp_product_bom_item (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    bom_id BIGINT NOT NULL COMMENT 'BOM ID',
    parent_product_id BIGINT NOT NULL COMMENT '父产品ID',
    child_product_id BIGINT NOT NULL COMMENT '子产品ID',
    child_product_name VARCHAR(255) NOT NULL COMMENT '子产品名称',
    child_product_spec VARCHAR(500) COMMENT '子产品规格',
    unit_id BIGINT COMMENT '单位ID',
    quantity DECIMAL(20,4) NOT NULL COMMENT '用量',
    loss_rate DECIMAL(8,4) DEFAULT 0 COMMENT '损耗率',
    effective_quantity DECIMAL(20,4) GENERATED ALWAYS AS (quantity * (1 + loss_rate)) STORED COMMENT '有效用量',
    is_key_material TINYINT DEFAULT 0 COMMENT '是否关键物料',
    is_alternative TINYINT DEFAULT 0 COMMENT '是否替代料',
    alternative_group VARCHAR(50) COMMENT '替代料组',
    position INT DEFAULT 1 COMMENT '位号',
    process_id BIGINT COMMENT '工序ID',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_bom (tenant_id, bom_id),
    INDEX idx_parent (parent_product_id),
    INDEX idx_child (child_product_id),
    INDEX idx_process (process_id)
) COMMENT='BOM明细表';
```

#### 2. 工艺路线相关表
```sql
-- 工艺路线主表
CREATE TABLE erp_process_route (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    route_no VARCHAR(50) NOT NULL COMMENT '工艺路线编号',
    route_name VARCHAR(255) NOT NULL COMMENT '工艺路线名称',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    version VARCHAR(20) DEFAULT '1.0' COMMENT '版本号',
    standard_cycle_time INT COMMENT '标准周期时间（分钟）',
    standard_labor_cost DECIMAL(20,2) COMMENT '标准人工成本',
    standard_overhead_cost DECIMAL(20,2) COMMENT '标准制造费用',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-生效，3-失效',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_route (tenant_id, route_no),
    INDEX idx_product (product_id),
    INDEX idx_status (status)
) COMMENT='工艺路线主表';

-- 工艺路线明细表
CREATE TABLE erp_process_route_item (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    route_id BIGINT NOT NULL COMMENT '工艺路线ID',
    process_id BIGINT NOT NULL COMMENT '工序ID',
    sequence INT NOT NULL COMMENT '序号',
    operation_name VARCHAR(255) NOT NULL COMMENT '工序名称',
    standard_time INT NOT NULL COMMENT '标准工时（分钟）',
    setup_time INT DEFAULT 0 COMMENT '准备时间（分钟）',
    worker_count INT DEFAULT 1 COMMENT '人员数量',
    equipment_id BIGINT COMMENT '设备ID',
    work_center_id BIGINT COMMENT '工作中心ID',
    labor_rate DECIMAL(10,2) COMMENT '人工费率',
    overhead_rate DECIMAL(10,2) COMMENT '制造费率',
    is_bottleneck TINYINT DEFAULT 0 COMMENT '是否瓶颈工序',
    quality_check_required TINYINT DEFAULT 0 COMMENT '是否需要质检',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_route (tenant_id, route_id),
    INDEX idx_process (process_id),
    INDEX idx_equipment (equipment_id),
    INDEX idx_sequence (sequence)
) COMMENT='工艺路线明细表';
```

#### 3. 生产排程相关表
```sql
-- 生产排程主表
CREATE TABLE erp_production_schedule (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    schedule_no VARCHAR(50) NOT NULL COMMENT '排程单号',
    schedule_name VARCHAR(255) NOT NULL COMMENT '排程名称',
    schedule_type INT DEFAULT 1 COMMENT '排程类型：1-主排程，2-详细排程',
    planning_horizon_days INT DEFAULT 30 COMMENT '计划天数',
    start_date DATE NOT NULL COMMENT '计划开始日期',
    end_date DATE NOT NULL COMMENT '计划结束日期',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-已发布，3-执行中，4-已完成',
    total_orders INT DEFAULT 0 COMMENT '总订单数',
    total_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '总数量',
    total_work_hours INT DEFAULT 0 COMMENT '总工时',
    created_by VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_by VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_schedule (tenant_id, schedule_no),
    INDEX idx_status (status),
    INDEX idx_date_range (start_date, end_date)
) COMMENT='生产排程主表';

-- 排程明细表
CREATE TABLE erp_production_schedule_item (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    schedule_id BIGINT NOT NULL COMMENT '排程ID',
    production_order_id BIGINT NOT NULL COMMENT '生产订单ID',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    quantity DECIMAL(20,4) NOT NULL COMMENT '排程数量',
    planned_start_time DATETIME NOT NULL COMMENT '计划开始时间',
    planned_end_time DATETIME NOT NULL COMMENT '计划结束时间',
    work_center_id BIGINT COMMENT '工作中心ID',
    equipment_id BIGINT COMMENT '设备ID',
    process_sequence INT COMMENT '工序序号',
    priority INT DEFAULT 3 COMMENT '优先级',
    setup_time INT DEFAULT 0 COMMENT '准备时间',
    run_time INT NOT NULL COMMENT '运行时间',
    wait_time INT DEFAULT 0 COMMENT '等待时间',
    status INT DEFAULT 1 COMMENT '状态：1-已计划，2-已下达，3-进行中，4-已完成，5-已延迟',
    actual_start_time DATETIME COMMENT '实际开始时间',
    actual_end_time DATETIME COMMENT '实际结束时间',
    completion_rate DECIMAL(5,2) DEFAULT 0 COMMENT '完成率',
    delay_reason VARCHAR(500) COMMENT '延迟原因',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_schedule (tenant_id, schedule_id),
    INDEX idx_order (production_order_id),
    INDEX idx_work_center (work_center_id),
    INDEX idx_equipment (equipment_id),
    INDEX idx_time_range (planned_start_time, planned_end_time),
    INDEX idx_status (status)
) COMMENT='生产排程明细表';
```

#### 4. MRP运算相关表
```sql
-- MRP参数表
CREATE TABLE erp_mrp_params (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    param_name VARCHAR(100) NOT NULL COMMENT '参数名称',
    param_code VARCHAR(50) NOT NULL COMMENT '参数编码',
    param_value TEXT NOT NULL COMMENT '参数值',
    param_type INT DEFAULT 1 COMMENT '参数类型：1-字符串，2-数字，3-日期，4-布尔',
    description VARCHAR(500) COMMENT '参数描述',
    is_system TINYINT DEFAULT 0 COMMENT '是否系统参数',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_code (tenant_id, param_code),
    INDEX idx_system (is_system)
) COMMENT='MRP参数表';

-- MRP运算结果表
CREATE TABLE erp_mrp_result (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    run_no VARCHAR(50) NOT NULL COMMENT '运算批次号',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    warehouse_id BIGINT COMMENT '仓库ID',
    period_start_date DATE NOT NULL COMMENT '周期开始日期',
    period_end_date DATE NOT NULL COMMENT '周期结束日期',
    gross_requirement DECIMAL(20,4) DEFAULT 0 COMMENT '毛需求',
    scheduled_receipts DECIMAL(20,4) DEFAULT 0 COMMENT '计划接收量',
    on_hand_inventory DECIMAL(20,4) DEFAULT 0 COMMENT '现有库存',
    net_requirement DECIMAL(20,4) DEFAULT 0 COMMENT '净需求',
    planned_order_receipts DECIMAL(20,4) DEFAULT 0 COMMENT '计划订单接收量',
    planned_order_releases DECIMAL(20,4) DEFAULT 0 COMMENT '计划订单发放量',
    order_type INT DEFAULT 1 COMMENT '订单类型：1-生产订单，2-采购订单',
    lot_sizing_rule INT DEFAULT 1 COMMENT '批量规则：1-固定批量，2-按需，3-最小-最大',
    lead_time INT DEFAULT 0 COMMENT '提前期（天）',
    safety_stock DECIMAL(20,4) DEFAULT 0 COMMENT '安全库存',
    order_status INT DEFAULT 1 COMMENT '订单状态：1-建议，2-确认，3-下达',
    due_date DATE COMMENT '需求日期',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_tenant_run (tenant_id, run_no),
    INDEX idx_product (product_id),
    INDEX idx_warehouse (warehouse_id),
    INDEX idx_period (period_start_date, period_end_date),
    INDEX idx_due_date (due_date)
) COMMENT='MRP运算结果表';
```

### 🔧 核心服务实现

#### BOM管理服务
```java
@Service
@RequiredArgsConstructor
public class ProductBomServiceImpl implements ProductBomService {

    private final ProductBomMapper bomMapper;
    private final ProductBomItemMapper bomItemMapper;
    private final ProductMapper productMapper;

    /**
     * 创建BOM
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long createProductBom(ProductBomCreateReqVO createReqVO) {
        // 1. 参数校验
        validateCreateBomParams(createReqVO);

        // 2. 生成BOM编号
        String bomNo = generateBomNo();

        // 3. 创建BOM主表
        ProductBomDO bom = ProductBomConvert.INSTANCE.convert(createReqVO);
        bom.setBomNo(bomNo);
        bom.setStatus(ProductBomStatusEnum.DRAFT.getCode());

        // 4. 计算总成本
        BigDecimal totalCost = calculateTotalCost(createReqVO.getItems());
        bom.setTotalMaterialCost(totalCost);

        // 5. 插入主表
        bomMapper.insert(bom);

        // 6. 创建BOM明细
        createBomItems(bom.getId(), createReqVO.getItems());

        // 7. 更新产品表BOM标识
        updateProductBomFlag(createReqVO.getProductId(), true);

        // 8. 发送通知
        sendBomCreateNotification(bom);

        return bom.getId();
    }

    /**
     * BOM展开 - 递归展开所有子物料
     */
    @Override
    public List<ProductBomExpandRespVO> expandBom(Long productId, int expandLevel) {
        List<ProductBomExpandRespVO> result = new ArrayList<>();
        expandBomRecursive(productId, 1, expandLevel, new BigDecimal("1"), result);
        return result;
    }

    /**
     * BOM成本计算
     */
    @Override
    public ProductBomCostRespVO calculateBomCost(Long bomId) {
        ProductBomDO bom = validateBomExists(bomId);
        List<ProductBomItemDO> items = bomItemMapper.selectListByBomId(bomId);

        ProductBomCostRespVO costResp = new ProductBomCostRespVO();
        costResp.setBomId(bomId);
        costResp.setBomNo(bom.getBomNo());

        BigDecimal totalMaterialCost = BigDecimal.ZERO;
        BigDecimal totalLaborCost = BigDecimal.ZERO;
        BigDecimal totalOverheadCost = BigDecimal.ZERO;

        for (ProductBomItemDO item : items) {
            ProductBomItemCostVO itemCost = calculateItemCost(item);
            costResp.getItems().add(itemCost);

            totalMaterialCost = totalMaterialCost.add(itemCost.getMaterialCost());
            totalLaborCost = totalLaborCost.add(itemCost.getLaborCost());
            totalOverheadCost = totalOverheadCost.add(itemCost.getOverheadCost());
        }

        costResp.setTotalMaterialCost(totalMaterialCost);
        costResp.setTotalLaborCost(totalLaborCost);
        costResp.setTotalOverheadCost(totalOverheadCost);
        costResp.setTotalCost(totalMaterialCost.add(totalLaborCost).add(totalOverheadCost));

        return costResp;
    }

    /**
     * 递归展开BOM
     */
    private void expandBomRecursive(Long productId, int currentLevel, int maxLevel,
                                  BigDecimal parentQuantity, List<ProductBomExpandRespVO> result) {
        if (currentLevel > maxLevel) {
            return;
        }

        ProductDO product = productMapper.selectById(productId);
        if (product == null || !product.getIsBom()) {
            return;
        }

        ProductBomDO bom = bomMapper.selectByProductId(productId);
        if (bom == null) {
            return;
        }

        List<ProductBomItemDO> items = bomItemMapper.selectListByBomId(bom.getId());
        for (ProductBomItemDO item : items) {
            ProductBomExpandRespVO expandVO = ProductBomConvert.INSTANCE.convertExpand(item);
            expandVO.setLevel(currentLevel);
            expandVO.setParentQuantity(parentQuantity);
            expandVO.setTotalQuantity(item.getEffectiveQuantity().multiply(parentQuantity));

            result.add(expandVO);

            // 如果子产品也有BOM，继续展开
            if (currentLevel < maxLevel) {
                expandBomRecursive(item.getChildProductId(), currentLevel + 1, maxLevel,
                                 expandVO.getTotalQuantity(), result);
            }
        }
    }
}
```

#### MRP运算服务
```java
@Service
@RequiredArgsConstructor
public class MrpCalculationServiceImpl implements MrpCalculationService {

    private final MrpParamsMapper paramsMapper;
    private final MrpResultMapper resultMapper;
    private final ProductBomService bomService;
    private final StockService stockService;

    /**
     * 执行MRP运算
     */
    @Override
    @Async("mrpExecutor")
    @Transactional(rollbackFor = Exception.class)
    public MrpCalculationResultVO executeMrpCalculation(MrpCalculationReqVO calculationReqVO) {
        String runNo = generateRunNo();

        try {
            log.info("开始执行MRP运算，批次号：{}", runNo);

            // 1. 获取MRP参数
            MrpParams params = getMrpParams(calculationReqVO);

            // 2. 清理历史数据
            cleanHistoryData(runNo);

            // 3. 获取需求（销售订单、预测等）
            List<MrpRequirementDO> requirements = getRequirements(calculationReqVO, params);

            // 4. 获取库存和计划接收量
            List<MrpInventoryDO> inventories = getInventories(calculationReqVO);

            // 5. 执行MRP运算
            MrpCalculationResultVO result = executeMrpCore(runNo, requirements, inventories, params);

            // 6. 保存运算结果
            saveMrpResults(runNo, result.getResults());

            // 7. 生成采购建议和生产计划
            generateSuggestions(runNo, result.getResults());

            log.info("MRP运算完成，批次号：{}，耗时：{}ms", runNo, result.getCalculationTime());

            return result;

        } catch (Exception e) {
            log.error("MRP运算失败，批次号：{}", runNo, e);
            throw new MrpCalculationException("MRP运算失败：" + e.getMessage());
        }
    }

    /**
     * MRP核心运算逻辑
     */
    private MrpCalculationResultVO executeMrpCore(String runNo,
                                                 List<MrpRequirementDO> requirements,
                                                 List<MrpInventoryDO> inventories,
                                                 MrpParams params) {

        long startTime = System.currentTimeMillis();
        List<MrpResultDO> results = new ArrayList<>();

        // 按产品分组处理需求
        Map<Long, List<MrpRequirementDO>> requirementByProduct = requirements.stream()
                .collect(Collectors.groupingBy(MrpRequirementDO::getProductId));

        for (Map.Entry<Long, List<MrpRequirementDO>> entry : requirementByProduct.entrySet()) {
            Long productId = entry.getKey();
            List<MrpRequirementDO> productRequirements = entry.getValue();

            // 按时间排序需求
            productRequirements.sort(Comparator.comparing(MrpRequirementDO::getDueDate));

            // 获取库存信息
            MrpInventoryDO inventory = inventories.stream()
                    .filter(inv -> inv.getProductId().equals(productId))
                    .findFirst()
                    .orElse(new MrpInventoryDO(productId, BigDecimal.ZERO));

            // 计算净需求
            List<MrpResultDO> productResults = calculateNetRequirement(
                    productId, productRequirements, inventory, params);

            results.addAll(productResults);
        }

        // 递归处理BOM下层物料需求
        processBomDemands(results, params);

        long endTime = System.currentTimeMillis();

        return MrpCalculationResultVO.builder()
                .runNo(runNo)
                .results(results)
                .calculationTime(endTime - startTime)
                .success(true)
                .build();
    }

    /**
     * 计算净需求
     */
    private List<MrpResultDO> calculateNetRequirement(Long productId,
                                                   List<MrpRequirementDO> requirements,
                                                   MrpInventoryDO inventory,
                                                   MrpParams params) {
        List<MrpResultDO> results = new ArrayList<>();

        BigDecimal onHandInventory = inventory.getOnHandInventory();
        BigDecimal scheduledReceipts = BigDecimal.ZERO;

        for (MrpRequirementDO requirement : requirements) {
            LocalDate periodDate = requirement.getDueDate();

            // 计算该周期内的计划接收量
            BigDecimal periodReceipts = getScheduledReceipts(productId, periodDate);

            // 毛需求 = 现有需求 + 计划接收量
            BigDecimal grossRequirement = requirement.getQuantity().add(periodReceipts);

            // 净需求 = 毛需求 - 现有库存 - 安全库存
            BigDecimal safetyStock = getSafetyStock(productId, params);
            BigDecimal netRequirement = grossRequirement.subtract(onHandInventory)
                    .subtract(safetyStock);

            // 如果净需求大于0，需要计划订单
            if (netRequirement.compareTo(BigDecimal.ZERO) > 0) {
                // 应用批量规则
                BigDecimal orderQuantity = applyLotSizingRule(netRequirement, productId, params);

                // 创建计划订单接收
                MrpResultDO plannedReceipt = createMrpResult(
                        runNo, productId, requirement.getWarehouseId(),
                        periodDate, grossRequirement, periodReceipts,
                        onHandInventory, netRequirement, orderQuantity,
                        BigDecimal.ZERO, 1, params);
                results.add(plannedReceipt);

                // 创建计划订单发放（考虑提前期）
                LocalDate releaseDate = periodDate.minusDays(getLeadTime(productId, params));
                MrpResultDO plannedRelease = createMrpResult(
                        runNo, productId, requirement.getWarehouseId(),
                        releaseDate, BigDecimal.ZERO, BigDecimal.ZERO,
                        BigDecimal.ZERO, BigDecimal.ZERO, orderQuantity,
                        orderQuantity, 1, params);
                results.add(plannedRelease);

                // 更新现有库存
                onHandInventory = onHandInventory.add(orderQuantity);
            }

            // 更新现有库存（减少）
            onHandInventory = onHandInventory.subtract(requirement.getQuantity());
        }

        return results;
    }
}
```

### 📊 Phase 2 交付清单

#### 后端交付物
- [ ] BOM管理服务（20个核心方法）
- [ ] 工艺路线管理服务（15个核心方法）
- [ ] MRP运算服务（复杂算法实现）
- [ ] 生产排程服务（智能排程算法）
- [ ] RESTful API接口（40个接口）
- [ ] 异步MRP计算（Spring异步处理）

#### 前端交付物
- [ ] BOM管理界面（树形展示、成本计算）
- [ ] 工艺路线管理界面（工序配置、时间管理）
- [ ] MRP运算界面（参数设置、结果展示）
- [ ] 生产排程界面（甘特图、资源调度）
- [ ] 物料需求分析报表

#### 技术亮点
- [ ] BOM递归展开算法优化
- [ ] MRP批量计算性能优化（异步+分页）
- [ ] 智能排程算法（约束满足问题）
- [ ] 前端树形组件和甘特图组件

---

## 🎯 Phase 3: 生产执行与质量管理 - 详细方案

### 📅 时间安排：6周（42天）

### 🎯 核心目标
实现生产过程实时监控、质量管理和设备管理

### 🗄️ 数据库详细设计

#### 1. 工序管理相关表
```sql
-- 工单主表
CREATE TABLE erp_work_order (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    work_order_no VARCHAR(50) NOT NULL COMMENT '工单号',
    production_order_id BIGINT NOT NULL COMMENT '生产订单ID',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    product_name VARCHAR(255) NOT NULL COMMENT '产品名称',
    quantity DECIMAL(20,4) NOT NULL COMMENT '工单数量',
    completed_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '完成数量',
    qualified_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '合格数量',
    work_center_id BIGINT NOT NULL COMMENT '工作中心ID',
    supervisor_id BIGINT COMMENT '主管ID',
    planned_start_time DATETIME NOT NULL COMMENT '计划开始时间',
    planned_end_time DATETIME NOT NULL COMMENT '计划结束时间',
    actual_start_time DATETIME COMMENT '实际开始时间',
    actual_end_time DATETIME COMMENT '实际结束时间',
    status INT DEFAULT 1 COMMENT '状态：1-已创建，2-已下达，3-进行中，4-已暂停，5-已完成，6-已取消',
    priority INT DEFAULT 3 COMMENT '优先级',
    instruction TEXT COMMENT '作业指导书',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_work_order (tenant_id, work_order_no),
    INDEX idx_production_order (production_order_id),
    INDEX idx_work_center (work_center_id),
    INDEX idx_status (status),
    INDEX idx_time_range (planned_start_time, planned_end_time)
) COMMENT='工单主表';

-- 工单进度表
CREATE TABLE erp_work_order_progress (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    work_order_id BIGINT NOT NULL COMMENT '工单ID',
    process_id BIGINT NOT NULL COMMENT '工序ID',
    process_name VARCHAR(255) NOT NULL COMMENT '工序名称',
    sequence INT NOT NULL COMMENT '工序序号',
    planned_start_time DATETIME COMMENT '计划开始时间',
    planned_end_time DATETIME COMMENT '计划结束时间',
    actual_start_time DATETIME COMMENT '实际开始时间',
    actual_end_time DATETIME COMMENT '实际结束时间',
    planned_quantity DECIMAL(20,4) COMMENT '计划数量',
    completed_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '完成数量',
    qualified_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '合格数量',
    rejected_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '不合格数量',
    scrap_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '报废数量',
    status INT DEFAULT 1 COMMENT '状态：1-待开始，2-进行中，3-已完成，4-异常',
    operator_id BIGINT COMMENT '操作员ID',
    equipment_id BIGINT COMMENT '设备ID',
    work_time INT DEFAULT 0 COMMENT '实际工时（分钟）',
    downtime INT DEFAULT 0 COMMENT '停机时间（分钟）',
    quality_status INT DEFAULT 1 COMMENT '质检状态：1-待检，2-合格，3-不合格',
    remark VARCHAR(500) COMMENT '备注',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    INDEX idx_tenant_work_order (tenant_id, work_order_id),
    INDEX idx_process (process_id),
    INDEX idx_operator (operator_id),
    INDEX idx_equipment (equipment_id),
    INDEX idx_status (status)
) COMMENT='工单进度表';
```

#### 2. 质量管理相关表
```sql
-- 质检标准表
CREATE TABLE erp_quality_standard (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    standard_no VARCHAR(50) NOT NULL COMMENT '标准编号',
    standard_name VARCHAR(255) NOT NULL COMMENT '标准名称',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    process_id BIGINT COMMENT '工序ID',
    inspection_type INT DEFAULT 1 COMMENT '检验类型：1-进料检验，2-过程检验，3-成品检验',
    standard_version VARCHAR(20) DEFAULT '1.0' COMMENT '标准版本',
    aql_level VARCHAR(10) COMMENT 'AQL水平',
    sampling_method VARCHAR(100) COMMENT '抽样方法',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-生效，3-失效',
    description TEXT COMMENT '标准描述',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_standard (tenant_id, standard_no),
    INDEX idx_product (product_id),
    INDEX idx_process (process_id),
    INDEX idx_status (status)
) COMMENT='质检标准表';

-- 质检项目表
CREATE TABLE erp_quality_item (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    standard_id BIGINT NOT NULL COMMENT '标准ID',
    item_no VARCHAR(50) NOT NULL COMMENT '项目编号',
    item_name VARCHAR(255) NOT NULL COMMENT '项目名称',
    item_type INT DEFAULT 1 COMMENT '项目类型：1-定性，2-定量',
    test_method VARCHAR(500) COMMENT '检验方法',
    standard_value VARCHAR(500) COMMENT '标准值',
    tolerance VARCHAR(100) COMMENT '公差范围',
    unit VARCHAR(20) COMMENT '单位',
    is_key_item TINYINT DEFAULT 0 COMMENT '是否关键项',
    sequence INT DEFAULT 1 COMMENT '检验序号',
    remark VARCHAR(500) COMMENT '备注',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_standard (tenant_id, standard_id),
    INDEX idx_key_item (is_key_item),
    INDEX idx_sequence (sequence)
) COMMENT='质检项目表';

-- 质检记录表
CREATE TABLE erp_quality_inspection (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    inspection_no VARCHAR(50) NOT NULL COMMENT '检验单号',
    batch_no VARCHAR(50) COMMENT '批次号',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    process_id BIGINT COMMENT '工序ID',
    work_order_id BIGINT COMMENT '工单ID',
    inspection_type INT DEFAULT 1 COMMENT '检验类型：1-进料检验，2-过程检验，3-成品检验',
    inspection_level INT DEFAULT 1 COMMENT '检验级别：1-全检，2-抽检',
    lot_size INT COMMENT '批量大小',
    sample_size INT COMMENT '样本数量',
    qualified_quantity INT DEFAULT 0 COMMENT '合格数量',
    rejected_quantity INT DEFAULT 0 COMMENT '不合格数量',
    scrap_quantity INT DEFAULT 0 COMMENT '报废数量',
    inspection_result INT DEFAULT 1 COMMENT '检验结果：1-合格，2-不合格，3-待复检',
    inspector_id BIGINT COMMENT '检验员ID',
    inspection_time DATETIME NOT NULL COMMENT '检验时间',
    environment VARCHAR(200) COMMENT '检验环境',
    equipment VARCHAR(200) COMMENT '检验设备',
    remark TEXT COMMENT '检验备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_inspection (tenant_id, inspection_no),
    INDEX idx_batch_no (batch_no),
    INDEX idx_product (product_id),
    INDEX idx_work_order (work_order_id),
    INDEX idx_inspection_time (inspection_time)
) COMMENT='质检记录表';

-- 质检明细表
CREATE TABLE erp_quality_inspection_item (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    inspection_id BIGINT NOT NULL COMMENT '检验ID',
    item_id BIGINT NOT NULL COMMENT '检验项目ID',
    sample_no VARCHAR(50) COMMENT '样本编号',
    measured_value VARCHAR(500) COMMENT '测量值',
    actual_value DECIMAL(20,6) COMMENT '实际数值',
    test_result INT DEFAULT 1 COMMENT '检验结果：1-合格，2-不合格，3-超差',
    defect_type VARCHAR(100) COMMENT '缺陷类型',
    defect_description VARCHAR(500) COMMENT '缺陷描述',
    image_urls TEXT COMMENT '缺陷图片URLs',
    inspector_id BIGINT COMMENT '检验员ID',
    inspection_time DATETIME NOT NULL COMMENT '检验时间',
    remark VARCHAR(500) COMMENT '备注',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    INDEX idx_tenant_inspection (tenant_id, inspection_id),
    INDEX idx_item (item_id),
    INDEX inspector (inspector_id),
    INDEX idx_result (test_result)
) COMMENT='质检明细表';
```

#### 3. 设备管理相关表
```sql
-- 设备台账表
CREATE TABLE erp_equipment (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    equipment_no VARCHAR(50) NOT NULL COMMENT '设备编号',
    equipment_name VARCHAR(255) NOT NULL COMMENT '设备名称',
    equipment_type VARCHAR(100) COMMENT '设备类型',
    model VARCHAR(100) COMMENT '设备型号',
    manufacturer VARCHAR(255) COMMENT '制造商',
    serial_number VARCHAR(100) COMMENT '序列号',
    purchase_date DATE COMMENT '购置日期',
    purchase_price DECIMAL(20,2) COMMENT '购置价格',
    service_life INT COMMENT '设计寿命（年）',
    work_center_id BIGINT COMMENT '工作中心ID',
    location VARCHAR(200) COMMENT '设备位置',
    capacity DECIMAL(20,4) COMMENT '产能（小时/天）',
    efficiency_rate DECIMAL(5,2) DEFAULT 100 COMMENT '效率系数',
    status INT DEFAULT 1 COMMENT '状态：1-正常，2-维修中，3-故障，4-报废',
    responsible_person VARCHAR(64) COMMENT '责任人',
    specification TEXT COMMENT '技术规格',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_equipment (tenant_id, equipment_no),
    INDEX idx_type (equipment_type),
    INDEX idx_work_center (work_center_id),
    INDEX idx_status (status)
) COMMENT='设备台账表';

-- 设备状态记录表
CREATE TABLE erp_equipment_status (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    equipment_id BIGINT NOT NULL COMMENT '设备ID',
    status INT NOT NULL COMMENT '状态：1-运行，2-待机，3-故障，4-维修，5-停机',
    status_start_time DATETIME NOT NULL COMMENT '状态开始时间',
    status_end_time DATETIME COMMENT '状态结束时间',
    duration INT COMMENT '持续时间（分钟）',
    work_order_id BIGINT COMMENT '关联工单ID',
    operator_id BIGINT COMMENT '操作员ID',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_tenant_equipment (tenant_id, equipment_id),
    INDEX idx_status (status),
    INDEX idx_time_range (status_start_time, status_end_time),
    INDEX idx_work_order (work_order_id)
) COMMENT='设备状态记录表';
```

### 🔧 核心服务实现

#### 工单管理服务
```java
@Service
@RequiredArgsConstructor
public class WorkOrderServiceImpl implements WorkOrderService {

    private final WorkOrderMapper workOrderMapper;
    private final WorkOrderProgressMapper progressMapper;
    private final ProductionOrderService productionOrderService;
    private final MessageService messageService;

    /**
     * 创建工单
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long createWorkOrder(WorkOrderCreateReqVO createReqVO) {
        // 1. 参数校验
        validateCreateWorkOrderParams(createReqVO);

        // 2. 生成工单号
        String workOrderNo = generateWorkOrderNo();

        // 3. 创建工单主表
        WorkOrderDO workOrder = WorkOrderConvert.INSTANCE.convert(createReqVO);
        workOrder.setWorkOrderNo(workOrderNo);
        workOrder.setStatus(WorkOrderStatusEnum.CREATED.getCode());

        // 4. 插入工单
        workOrderMapper.insert(workOrder);

        // 5. 创建工单工序进度
        createWorkOrderProgress(workOrder.getId(), createReqVO);

        // 6. 发送工单创建通知
        sendWorkOrderNotification(workOrder, "工单已创建");

        return workOrder.getId();
    }

    /**
     * 开始工单生产
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void startWorkOrder(Long workOrderId, WorkOrderStartReqVO startReqVO) {
        WorkOrderDO workOrder = validateWorkOrder(workOrderId);

        // 1. 状态校验
        if (!WorkOrderStatusEnum.ISSUED.getCode().equals(workOrder.getStatus())) {
            throw exception(WORK_ORDER_STATUS_ERROR);
        }

        // 2. 检查设备状态
        checkEquipmentStatus(startReqVO.getEquipmentId());

        // 3. 检查物料状态
        checkMaterialAvailability(workOrderId);

        // 4. 更新工单状态
        WorkOrderDO updateDO = new WorkOrderDO();
        updateDO.setId(workOrderId);
        updateDO.setStatus(WorkOrderStatusEnum.IN_PROGRESS.getCode());
        updateDO.setActualStartTime(LocalDateTime.now());
        workOrderMapper.updateById(updateDO);

        // 5. 开始第一道工序
        startFirstProcess(workOrderId, startReqVO);

        // 6. 更新设备状态
        updateEquipmentStatus(startReqVO.getEquipmentId(), EquipmentStatusEnum.RUNNING);

        // 7. 发送开始生产通知
        sendWorkOrderNotification(workOrder, "工单已开始生产");
    }

    /**
     * 完成工序
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void completeProcess(Long workOrderId, Long processId, WorkOrderProcessCompleteReqVO completeReqVO) {
        // 1. 获取工单和工序进度
        WorkOrderDO workOrder = validateWorkOrder(workOrderId);
        WorkOrderProgressDO progress = getWorkOrderProgress(workOrderId, processId);

        // 2. 质量检验（如果需要）
        if (completeReqVO.getRequireQualityCheck()) {
            performQualityCheck(workOrderId, processId, completeReqVO.getQualityData());
        }

        // 3. 更新工序进度
        updateProcessProgress(progress, completeReqVO);

        // 4. 物料消耗记录
        recordMaterialConsumption(workOrderId, processId, completeReqVO.getMaterialConsumption());

        // 5. 判断是否还有下一道工序
        WorkOrderProgressDO nextProcess = getNextProcess(workOrderId, processId);
        if (nextProcess != null) {
            // 开始下一道工序
            startNextProcess(nextProcess, completeReqVO);
        } else {
            // 所有工序完成，完成工单
            completeWorkOrder(workOrderId);
        }

        // 6. 更新设备状态
        if (completeReqVO.getEquipmentId() != null) {
            updateEquipmentStatus(completeReqVO.getEquipmentId(), EquipmentStatusEnum.IDLE);
        }

        // 7. 发送进度更新通知
        sendProgressUpdateNotification(workOrder, progress);
    }

    /**
     * 实时进度更新
     */
    @Override
    @Async("workOrderExecutor")
    public void updateRealTimeProgress(Long workOrderId, Long processId,
                                     BigDecimal currentQuantity, String status) {
        // 1. 获取工单进度
        WorkOrderProgressDO progress = getWorkOrderProgress(workOrderId, processId);

        // 2. 更新实时进度
        progress.setCompletedQuantity(currentQuantity);
        progress.setUpdateTime(LocalDateTime.now());

        // 3. 根据状态更新其他字段
        switch (status) {
            case "RUNNING":
                progress.setStatus(WorkOrderProcessStatusEnum.IN_PROGRESS.getCode());
                break;
            case "PAUSED":
                progress.setStatus(WorkOrderProcessStatusEnum.PAUSED.getCode());
                break;
            case "ERROR":
                progress.setStatus(WorkOrderProcessStatusEnum.ERROR.getCode());
                break;
        }

        progressMapper.updateById(progress);

        // 4. 通过WebSocket推送实时数据
        webSocketService.sendToTopic("/work-order/progress/" + workOrderId,
                                   Map.of("processId", processId,
                                         "quantity", currentQuantity,
                                         "status", status));
    }
}
```

### 📊 Phase 3 交付清单

#### 后端交付物
- [ ] 工单管理服务（20个核心方法）
- [ ] 质量管理服务（25个核心方法）
- [ ] 设备管理服务（15个核心方法）
- [ ] 生产监控服务（实时数据采集）
- [ ] WebSocket实时推送服务
- [ ] RESTful API接口（60个接口）

#### 前端交付物
- [ ] 工单管理界面（甘特图、进度追踪）
- [ ] 质检管理界面（抽检方案、记录录入）
- [ ] 设备管理界面（OEE计算、维护计划）
- [ ] 生产监控看板（大屏展示、实时数据）
- [ ] 移动端APP（工序确认、质检录入）

#### 技术亮点
- [ ] WebSocket实时数据推送
- [ ] 质检数据统计分析算法
- [ ] 设备OEE实时计算
- [ ] 前端实时图表和进度条组件

---

## 🎯 Phase 4: 成本核算与数据分析 - 详细方案

### 📅 时间安排：5周（35天）

### 🎯 核心目标
构建完整的生产成本核算体系与数据分析平台

### 🗄️ 数据��详��设计

#### 1. 成本核算相关表
```sql
-- 标准成本表
CREATE TABLE erp_cost_standard (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    version VARCHAR(20) DEFAULT '1.0' COMMENT '版本号',
    effective_date DATETIME NOT NULL COMMENT '生效日期',
    expire_date DATETIME COMMENT '失效日期',
    material_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '材料成本',
    labor_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '人工成本',
    overhead_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '制造费用',
    total_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '总成本',
    cost_currency VARCHAR(10) DEFAULT 'CNY' COMMENT '成本币种',
    calculation_date DATETIME NOT NULL COMMENT '计算日期',
    bom_version VARCHAR(20) COMMENT '关联BOM版本',
    route_version VARCHAR(20) COMMENT '关联工艺版本',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-生效，3-失效',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_product (tenant_id, product_id),
    INDEX idx_effective_date (effective_date),
    INDEX idx_status (status)
) COMMENT='标准成本表';

-- 实际成本表
CREATE TABLE erp_cost_actual (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    cost_no VARCHAR(50) NOT NULL COMMENT '成本单号',
    work_order_id BIGINT COMMENT '工单ID',
    production_order_id BIGINT COMMENT '生产订单ID',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    production_quantity DECIMAL(20,4) NOT NULL COMMENT '生产数量',
    material_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '材料成本',
    material_cost_adjust DECIMAL(20,2) DEFAULT 0 COMMENT '材料成本调整',
    labor_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '人工成本',
    labor_cost_adjust DECIMAL(20,2) DEFAULT 0 COMMENT '人工成本调整',
    overhead_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '制造费用',
    overhead_cost_adjust DECIMAL(20,2) DEFAULT 0 COMMENT '制造费用调整',
    total_cost DECIMAL(20,2) NOT NULL DEFAULT 0 COMMENT '总成本',
    unit_cost DECIMAL(20,4) NOT NULL DEFAULT 0 COMMENT '单位成本',
    cost_currency VARCHAR(10) DEFAULT 'CNY' COMMENT '成本币种',
    cost_period VARCHAR(20) NOT NULL COMMENT '成本期间（YYYY-MM）',
    calculation_date DATETIME NOT NULL COMMENT '计算日期',
    last_adjust_date DATETIME COMMENT '最后调整日期',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-已计算，3-已确认',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_cost (tenant_id, cost_no),
    INDEX idx_work_order (work_order_id),
    INDEX idx_production_order (production_order_id),
    INDEX idx_cost_period (cost_period),
    INDEX idx_status (status)
) COMMENT='实际成本表';

-- 成本差异分析表
CREATE TABLE erp_cost_variance (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    cost_actual_id BIGINT NOT NULL COMMENT '实际成本ID',
    product_id BIGINT NOT NULL COMMENT '产品ID',
    production_quantity DECIMAL(20,4) NOT NULL COMMENT '生产数量',
    standard_total_cost DECIMAL(20,2) NOT NULL COMMENT '标准总成本',
    actual_total_cost DECIMAL(20,2) NOT NULL COMMENT '实际总成本',
    total_variance DECIMAL(20,2) NOT NULL COMMENT '总差异',
    total_variance_rate DECIMAL(8,4) NOT NULL COMMENT '总差异率',
    material_variance DECIMAL(20,2) DEFAULT 0 COMMENT '材料成本差异',
    material_variance_rate DECIMAL(8,4) DEFAULT 0 COMMENT '材料差异率',
    labor_variance DECIMAL(20,2) DEFAULT 0 COMMENT '人工成本差异',
    labor_variance_rate DECIMAL(8,4) DEFAULT 0 COMMENT '人工差异率',
    overhead_variance DECIMAL(20,2) DEFAULT 0 COMMENT '制造费用差异',
    overhead_variance_rate DECIMAL(8,4) DEFAULT 0 COMMENT '制造费用差异率',
    variance_type INT DEFAULT 1 COMMENT '差异类型：1-有利，2-不利',
    analysis_date DATETIME NOT NULL COMMENT '分析日期',
    variance_reason VARCHAR(500) COMMENT '差异原因',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_tenant_actual (tenant_id, cost_actual_id),
    INDEX idx_product (product_id),
    INDEX idx_analysis_date (analysis_date),
    INDEX idx_variance_type (variance_type)
) COMMENT='成本差异分析表';

-- 工时统计表
CREATE TABLE erp_work_hours (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    work_order_id BIGINT NOT NULL COMMENT '工单ID',
    process_id BIGINT NOT NULL COMMENT '工序ID',
    operator_id BIGINT COMMENT '操作员ID',
    work_date DATE NOT NULL COMMENT '工作日期',
    shift_id BIGINT COMMENT '班次ID',
    start_time TIME COMMENT '开始时间',
    end_time TIME COMMENT '结束时间',
    duration INT NOT NULL COMMENT '工作时长（分钟）',
    standard_duration INT DEFAULT 0 COMMENT '标准工时（分钟）',
    overtime_duration INT DEFAULT 0 COMMENT '加班时长（分钟）',
    machine_hours DECIMAL(10,2) DEFAULT 0 COMMENT '机时',
    operator_rate DECIMAL(10,2) COMMENT '操作员工时费率',
    machine_rate DECIMAL(10,2) COMMENT '设备时费率',
    labor_cost DECIMAL(20,2) DEFAULT 0 COMMENT '人工成本',
    machine_cost DECIMAL(20,2) DEFAULT 0 COMMENT '设备成本',
    status INT DEFAULT 1 COMMENT '状态：1-有效，2-无效',
    remark VARCHAR(500) COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    INDEX idx_tenant_work_order (tenant_id, work_order_id),
    INDEX idx_process (process_id),
    INDEX idx_operator (operator_id),
    INDEX idx_work_date (work_date),
    INDEX idx_status (status)
) COMMENT='工时统计表';
```

#### 2. 生产数据分析相关表
```sql
-- 生产KPI表
CREATE TABLE erp_production_kpi (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    kpi_no VARCHAR(50) NOT NULL COMMENT 'KPI编号',
    kpi_name VARCHAR(255) NOT NULL COMMENT 'KPI名称',
    kpi_type INT NOT NULL COMMENT 'KPI类型：1-OEE，2-合格率，3-达成率，4-交期率',
    category INT DEFAULT 1 COMMENT '分类：1-效率，2-质量，3-交付，4-成本',
    work_center_id BIGINT COMMENT '工作中心ID',
    product_id BIGINT COMMENT '产品ID',
    measurement_period INT NOT NULL COMMENT '统计周期：1-小时，2-天，3-周，4-月',
    target_value DECIMAL(20,4) COMMENT '目标值',
    actual_value DECIMAL(20,4) NOT NULL COMMENT '实际值',
    variance DECIMAL(20,4) COMMENT '差异值',
    variance_rate DECIMAL(8,4) COMMENT '差异率',
    calculation_date DATETIME NOT NULL COMMENT '计算日期',
    period_start_time DATETIME NOT NULL COMMENT '周期开始时间',
    period_end_time DATETIME NOT NULL COMMENT '周期结束时间',
    data_source VARCHAR(100) COMMENT '数据来源',
    remarks TEXT COMMENT '备注',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_tenant_kpi (tenant_id, kpi_no),
    INDEX idx_type (kpi_type),
    INDEX idx_work_center (work_center_id),
    INDEX idx_product (product_id),
    INDEX idx_calculation_date (calculation_date)
) COMMENT='生产KPI表';

-- 生产报表表
CREATE TABLE erp_production_report (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    report_no VARCHAR(50) NOT NULL COMMENT '报表编号',
    report_name VARCHAR(255) NOT NULL COMMENT '报表名称',
    report_type INT NOT NULL COMMENT '报表类型：1-日报，2-周报，3-月报，4-年报',
    report_period VARCHAR(50) NOT NULL COMMENT '报表期间',
    work_center_id BIGINT COMMENT '工作中心ID',
    production_orders INT DEFAULT 0 COMMENT '生产订单数',
    total_plan_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '总计划数量',
    total_completed_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '总完成数量',
    total_qualified_quantity DECIMAL(20,4) DEFAULT 0 COMMENT '总合格数量',
    completion_rate DECIMAL(5,2) DEFAULT 0 COMMENT '完成率',
    quality_rate DECIMAL(5,2) DEFAULT 0 COMMENT '合格率',
    total_work_hours DECIMAL(20,2) DEFAULT 0 COMMENT '总工时',
    total_equipment_hours DECIMAL(20,2) DEFAULT 0 COMMENT '总机时',
    oee DECIMAL(5,2) DEFAULT 0 COMMENT 'OEE',
    on_time_delivery_rate DECIMAL(5,2) DEFAULT 0 COMMENT '准时交付率',
    production_value DECIMAL(20,2) DEFAULT 0 COMMENT '产值',
    average_cost DECIMAL(20,4) DEFAULT 0 COMMENT '平均成本',
    report_date DATE NOT NULL COMMENT '报表日期',
    generate_time DATETIME NOT NULL COMMENT '生成时间',
    status INT DEFAULT 1 COMMENT '状态：1-草稿，2-已发布',
    report_data TEXT COMMENT '详细数据（JSON）',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    INDEX idx_tenant_report (tenant_id, report_no),
    INDEX idx_type (report_type),
    INDEX idx_work_center (work_center_id),
    INDEX idx_report_date (report_date)
) COMMENT='生产报表表';

-- 看板配置表
CREATE TABLE erp_production_dashboard_config (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    tenant_id BIGINT NOT NULL COMMENT '租户ID',
    config_name VARCHAR(255) NOT NULL COMMENT '配置名称',
    config_type INT DEFAULT 1 COMMENT '配置类型：1-大屏，2-PC端，3-移动端',
    screen_resolution VARCHAR(50) COMMENT '屏幕分辨率',
    layout_config TEXT NOT NULL COMMENT '布局配置（JSON）',
    component_config TEXT NOT NULL COMMENT '组件配置（JSON）',
    data_refresh_interval INT DEFAULT 5 COMMENT '数据刷新间隔（秒）',
    is_default TINYINT DEFAULT 0 COMMENT '是否默认配置',
    is_active TINYINT DEFAULT 1 COMMENT '是否启用',
    creator VARCHAR(64) COMMENT '创建人',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updater VARCHAR(64) COMMENT '更新人',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '是否删除',

    INDEX idx_tenant_config (tenant_id, config_name),
    INDEX idx_type (config_type),
    INDEX idx_active (is_active)
) COMMENT='看板配置表';
```

### 🔧 核心服务实现

#### 成本核算服务
```java
@Service
@RequiredArgsConstructor
public class CostAccountingServiceImpl implements CostAccountingService {

    private final CostActualMapper actualMapper;
    private final CostStandardMapper standardMapper;
    private readonly CostVarianceMapper varianceMapper;
    private final WorkHoursMapper workHoursMapper;
    private final WorkOrderService workOrderService;

    /**
     * 计算实际成本
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void calculateActualCost(CostCalculationReqVO calculationReqVO) {
        // 1. 生成成本单号
        String costNo = generateCostNo();

        try {
            // 2. 获取工单信息
            WorkOrderDO workOrder = workOrderService.getWorkOrder(calculationReqVO.getWorkOrderId());

            // 3. 计算材料成本
            BigDecimal materialCost = calculateMaterialCost(calculationReqVO);

            // 4. 计算人工成本
            BigDecimal laborCost = calculateLaborCost(calculationReqVO);

            // 5. 计算制造费用
            BigDecimal overheadCost = calculateOverheadCost(calculationReqVO);

            // 6. 创建实际成本记录
            CostActualDO actualCost = new CostActualDO();
            actualCost.setCostNo(costNo);
            actualCost.setWorkOrderId(calculationReqVO.getWorkOrderId());
            actualCost.setProductionOrderId(workOrder.getProductionOrderId());
            actualCost.setProductId(workOrder.getProductId());
            actualCost.setProductionQuantity(workOrder.getCompletedQuantity());

            actualCost.setMaterialCost(materialCost);
            actualCost.setLaborCost(laborCost);
            actualCost.setOverheadCost(overheadCost);
            actualCost.setTotalCost(materialCost.add(laborCost).add(overheadCost));
            actualCost.setUnitCost(calculateUnitCost(workOrder.getCompletedQuantity(), materialCost.add(laborCost).add(overheadCost)));

            actualCost.setCostPeriod(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM")));
            actualCost.setCalculationDate(LocalDateTime.now());
            actualCost.setStatus(CostActualStatusEnum.CALCULATED.getCode());

            actualMapper.insert(actualCost);

            // 7. 进行成本差异分析
            performVarianceAnalysis(actualCost);

            // 8. 更新工单状态
            updateWorkOrderCostStatus(workOrder.getId(), actualCost.getId());

            log.info("成本计算完成，成本单号：{}", costNo);

        } catch (Exception e) {
            log.error("成本计算失败，成本单号：{}", costNo, e);
            throw new CostCalculationException("成本计算失败：" + e.getMessage());
        }
    }

    /**
     * 计算材料成本
     */
    private BigDecimal calculateMaterialCost(CostCalculationReqVO calculationReqVO) {
        BigDecimal totalMaterialCost = BigDecimal.ZERO;

        // 1. 获取工单物料消耗记录
        List<MaterialConsumptionDO> consumptions = materialConsumptionMapper
                .selectByWorkOrderId(calculationReqVO.getWorkOrderId());

        for (MaterialConsumptionDO consumption : consumptions) {
            // 2. 获取材料单价
            BigDecimal unitPrice = getMaterialUnitPrice(consumption.getProductId());

            // 3. 计算材料成本
            BigDecimal materialCost = consumption.getConsumedQuantity().multiply(unitPrice);

            totalMaterialCost = totalMaterialCost.add(materialCost);
        }

        // 4. 应用材料成本调整
        if (calculationReqVO.getMaterialAdjustAmount() != null) {
            totalMaterialCost = totalMaterialCost.add(calculationReqVO.getMaterialAdjustAmount());
        }

        return totalMaterialCost;
    }

    /**
     * 计算人工成本
     */
    private BigDecimal calculateLaborCost(CostCalculationReqVO calculationReqVO) {
        // 1. 获取工单工时统计
        List<WorkHoursDO> workHoursList = workHoursMapper
                .selectByWorkOrderId(calculationReqVO.getWorkOrderId());

        BigDecimal totalLaborCost = BigDecimal.ZERO;

        for (WorkHoursDO workHours : workHoursList) {
            // 2. 计算人工成本 = 工时 * 费率
            BigDecimal laborHours = BigDecimal.valueOf(workHours.getDuration())
                    .divide(BigDecimal.valueOf(60), 4, RoundingMode.HALF_UP); // 转换为小时

            BigDecimal laborCost = laborHours.multiply(workHours.getOperatorRate());
            totalLaborCost = totalLaborCost.add(laborCost);
        }

        // 3. 应用人工成本调整
        if (calculationReqVO.getLaborAdjustAmount() != null) {
            totalLaborCost = totalLaborCost.add(calculationReqVO.getLaborAdjustAmount());
        }

        return totalLaborCost;
    }

    /**
     * 计算制造费用
     */
    private BigDecimal calculateOverheadCost(CostCalculationReqVO calculationReqVO) {
        // 1. 获取工时统计
        List<WorkHoursDO> workHoursList = workHoursMapper
                .selectByWorkOrderId(calculationReqVO.getWorkOrderId());

        BigDecimal totalMachineCost = BigDecimal.ZERO;
        BigDecimal totalUtilityCost = BigDecimal.ZERO;
        BigDecimal totalOtherCost = BigDecimal.ZERO;

        for (WorkHoursDO workHours : workHoursList) {
            // 2. 设备成本
            BigDecimal machineHours = BigDecimal.valueOf(workHours.getMachineHours());
            totalMachineCost = totalMachineCost.add(machineHours.multiply(workHours.getMachineRate()));

            // 3. 水电费等制造费用（根据设备类型和工时计算）
            BigDecimal utilityCost = calculateUtilityCost(workHours);
            totalUtilityCost = totalUtilityCost.add(utilityCost);

            // 4. 其他制造费用
            BigDecimal otherCost = calculateOtherOverhead(workHours);
            totalOtherCost = totalOtherCost.add(otherCost);
        }

        BigDecimal totalOverheadCost = totalMachineCost.add(totalUtilityCost).add(totalOtherCost);

        // 5. 应用制造费用调整
        if (calculationReqVO.getOverheadAdjustAmount() != null) {
            totalOverheadCost = totalOverheadCost.add(calculationReqVO.getOverheadAdjustAmount());
        }

        return totalOverheadCost;
    }

    /**
     * 成本差异分析
     */
    @Transactional(rollbackFor = Exception.class)
    public void performVarianceAnalysis(CostActualDO actualCost) {
        try {
            // 1. 获取标准成本
            CostStandardDO standardCost = standardMapper.selectByProductId(actualCost.getProductId());

            if (standardCost == null) {
                log.warn("未找到产品[{}]的标准成本，无法进行差异分析", actualCost.getProductId());
                return;
            }

            // 2. 计算各项差异
            BigDecimal standardTotalCost = standardCost.getTotalCost()
                    .multiply(actualCost.getProductionQuantity());

            BigDecimal materialVariance = actualCost.getMaterialCost()
                    .subtract(standardCost.getMaterialCost().multiply(actualCost.getProductionQuantity()));

            BigDecimal laborVariance = actualCost.getLaborCost()
                    .subtract(standardCost.getLaborCost().multiply(actualCost.getProductionQuantity()));

            BigDecimal overheadVariance = actualCost.getOverheadCost()
                    .subtract(standardCost.getOverheadCost().multiply(actualCost.getProductionQuantity()));

            BigDecimal totalVariance = actualCost.getTotalCost().subtract(standardTotalCost);

            // 3. 创建差异分析记录
            CostVarianceDO variance = new CostVarianceDO();
            variance.setCostActualId(actualCost.getId());
            variance.setProductId(actualCost.getProductId());
            variance.setProductionQuantity(actualCost.getProductionQuantity());
            variance.setStandardTotalCost(standardTotalCost);
            variance.setActualTotalCost(actualCost.getTotalCost());
            variance.setTotalVariance(totalVariance);

            variance.setMaterialVariance(materialVariance);
            variance.setLaborVariance(laborVariance);
            variance.setOverheadVariance(overheadVariance);

            // 4. 计算差异率
            if (standardTotalCost.compareTo(BigDecimal.ZERO) > 0) {
                variance.setTotalVarianceRate(
                        totalVariance.divide(standardTotalCost, 4, RoundingMode.HALF_UP)
                                .multiply(BigDecimal.valueOf(100)));
                variance.setMaterialVarianceRate(
                        materialVariance.divide(standardCost.getMaterialCost(), 4, RoundingMode.HALF_UP)
                                .multiply(BigDecimal.valueOf(100)));
                variance.setLaborVarianceRate(
                        laborVariance.divide(standardCost.getLaborCost(), 4, RoundingMode.HALF_UP)
                                .multiply(BigDecimal.valueOf(100)));
                variance.setOverheadVarianceRate(
                        overheadVariance.divide(standardCost.getOverheadCost(), 4, RoundingMode.HALF_UP)
                                .multiply(BigDecimal.valueOf(100)));
            }

            // 5. 判断差异类型
            variance.setVarianceType(totalVariance.compareTo(BigDecimal.ZERO) <= 0 ?
                                   CostVarianceTypeEnum.FAVORABLE.getCode() :
                                   CostVarianceTypeEnum.UNFAVORABLE.getCode());

            variance.setAnalysisDate(LocalDateTime.now());

            // 6. 分析差异原因
            variance.setVarianceReason(analyzeVarianceReasons(materialVariance, laborVariance, overheadVariance));

            varianceMapper.insert(variance);

            // 7. 发送差异预警（如果差异过大）
            if (Math.abs(variance.getTotalVarianceRate().doubleValue()) > 10) { // 差异超过10%
                sendVarianceAlert(actualCost, variance);
            }

        } catch (Exception e) {
            log.error("成本差异分析失败，实际成本ID：{}", actualCost.getId(), e);
        }
    }

    /**
     * 生成生产报表
     */
    @Override
    @Async("reportExecutor")
    public void generateProductionReport(ProductionReportReqVO reportReqVO) {
        try {
            log.info("开始生成生产报表：{}", reportReqVO);

            // 1. 生成报表编号
            String reportNo = generateReportNo();

            // 2. 获取数据
            List<WorkOrderDO> workOrders = workOrderService
                    .getWorkOrdersByPeriod(reportReqVO);

            // 3. 计算统计指标
            ProductionReportVO report = calculateReportMetrics(workOrders);

            // 4. 创建报表记录
            ProductionReportDO reportDO = new ProductionReportDO();
            reportDO.setReportNo(reportNo);
            reportDO.setReportName(generateReportName(reportReqVO));
            reportDO.setReportType(reportReqVO.getReportType());
            reportDO.setReportPeriod(reportReqVO.getPeriod());
            reportDO.setWorkCenterId(reportReqVO.getWorkCenterId());

            reportDO.setProductionOrders(report.getTotalOrders());
            reportDO.setTotalPlanQuantity(report.getTotalPlanQuantity());
            reportDO.setTotalCompletedQuantity(report.getTotalCompletedQuantity());
            reportDO.setTotalQualifiedQuantity(report.getTotalQualifiedQuantity());
            reportDO.setCompletionRate(report.getCompletionRate());
            reportDO.setQualityRate(report.getQualityRate());
            reportDO.setTotalWorkHours(report.getTotalWorkHours());
            reportDO.setOEE(report.getOEE());
            reportDO.setProductionValue(report.getProductionValue());
            reportDO.setAverageCost(report.getAverageCost());

            reportDO.setReportDate(LocalDate.now());
            reportDO.setGenerateTime(LocalDateTime.now());
            reportDO.setStatus(ProductionReportStatusEnum.DRAFT.getCode());
            reportDO.setReportData(JSON.toJSONString(report.getDetailData()));

            productionReportMapper.insert(reportDO);

            log.info("生产报表生成完成，报表编号：{}", reportNo);

        } catch (Exception e) {
            log.error("生产报表生成失败", e);
            throw new ReportGenerationException("生产报表生成失败：" + e.getMessage());
        }
    }
}
```

### 📊 Phase 4 交付清单

#### 后端交付物
- [ ] 成本核算服务（20个核心方法）
- [ ] 生产数据分析服务（15个核心方法）
- [ ] 报表生成服务（定时任务、异步生成）
- [ ] 数据可视化服务（图表数据处理）
- [ ] RESTful API接口（35个接口）
- [ ] 数据导出服务（Excel、PDF）

#### 前端交付物
- [ ] 成本核算界面（成本核算、差异分析）
- [ ] 生产报表界面（日报、周报、月报）
- [ ] 数据看板界面（大屏展示、实时数据）
- [ ] 移动端数据查询（成本查询、指标查看）
- [ ] 数据分析图表（趋势分析、对比分析）

#### 技术亮点
- [ ] 复杂成本核算算法
- [ ] 大数据量报表性能优化（分页查询、缓存）
- [ ] 数据可视化（ECharts、图表库）
- [ ] 异步报表生成（Spring Async + 消息队列）

---

## 🎯 整体技术架构总结

### 系统架构图
```
┌─────────────────────────────────────────────────────────────────┐
│                    ERP 生产管理系统                                 │
├─────────────────────────────────────────────────────────────────┤
│  前端层 (Vue3 + TypeScript + Element Plus + ECharts)              │
│  ┌──────────┬──────────┬──────────┬──────────┬──────────┐        │
│  │生产订单  │生产计划  │生产执行  │质量管理  │成本分析  │        │
│  │管理界面  │管理界面  │管理界面  │管理界面  │管理界面  │        │
│  └──────────┴──────────┴──────────┴──────────┴──────────┘        │
├─────────────────────���───────────────────────────────────────────┤
│  服务层 (Spring Boot + MyBatis-Plus)                              │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │  生产订单服务 (ProductionOrderService)                      │  │
│  ├────────────────────────────────────────────────────────────┤  │
│  │  生产计划服务 (ProductionPlanningService)                   │  │
│  ├────────────────────────────────────────────────────────────┤  │
│  │  生产执行服务 (ProductionExecutionService)                  │  │
│  ├────────────────────────────────────────────────────────────┤  │
│  │  质量管理服务 (QualityManagementService)                    │  │
│  ├────────────────────────────────────────────────────────────┤  │
│  │  成本核算服务 (CostAccountingService)                       │  │
│  └────────────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
│  消息队列 (RabbitMQ) - 异步处理                                     │
│  ├─ MRP计算队列、报表生成队列、成本计算队列、设备监控队列             │
├─────────────────────────────────────────────────────────────────┤
│  缓存层 (Redis)                                                   │
│  ├─ BOM数据缓存、生产数据缓存、KPI指标缓存                          │
├─────────────────────────────────────────────────────────────────┤
│  数据访问层 (MyBatis-Plus + Druid)                                 │
│  ├─ 分页查询优化、索引优化、读写分离                                  │
├─────────────────────────────────────────────────────────────────┤
│  数据库层 (MySQL 8.0 主从复制 + ShardingSphere 分库分表)           │
│  ├─ 现有模块：28张表                                                │
│  ├─ 生产模块：3张表（Phase1）                                       │
│  ├─ 计划模块：12张表（Phase2）                                      │
│  ├─ 执行模块：15张表（Phase3）                                      │
│  └─ 成本模块：8张表（Phase4）                                       │
└─────────────────────────────────────────────────────────────────┘
```

### 开发计划总览

| 阶段 | 时间周期 | 主要功能 | 数据库表数 | API接口数 | 前端页面数 | 团队人员 |
|------|----------|----------|------------|-----------|------------|----------|
| Phase 1 | 4周 | 生产订单管理 | 3张新表 | 25个 | 5个 | 6人 |
| Phase 2 | 5周 | 生产计划管理 | 12张新表 | 40个 | 8个 | 8人 |
| Phase 3 | 6周 | 生产执行与质量管理 | 15张新表 | 60个 | 10个 | 10人 |
| Phase 4 | 5周 | 成本核算与数据分析 | 8张新表 | 35个 | 7个 | 8人 |
| **总计** | **20周** | **完整生产管理系统** | **38张新表** | **160个** | **30个** | **10人** |

### 关键技术选型

| 技术类别 | 选型方案 | 版本 | 说明 |
|----------|----------|------|------|
| 后端框架 | Spring Boot | 3.x | 微服务架构 |
| ORM框架 | MyBatis-Plus | 3.5.x | 增强版MyBatis |
| 安全框架 | Spring Security | 6.x | 权限控制 |
| 工作流 | Activiti | 7.x | 流程审批 |
| 消息队列 | RabbitMQ | 3.12.x | 异步处理 |
| 缓存 | Redis | 7.x | 数据缓存 |
| 数据库 | MySQL | 8.0 | 主从复制 |
| 分库分表 | ShardingSphere | 5.x | 数据分片 |
| 前端框架 | Vue.js | 3.x | 响应式前端 |
| UI组件库 | Element Plus | 2.x | UI组件 |
| 图表库 | ECharts | 5.x | 数据可视化 |
| 构建工具 | Vite | 4.x | 快速构建 |

---

## 📞 文档信息

**文档名称**: 生产管理系统开发计划 - 细化版
**版本**: v4.0
**创建日期**: 2025-11-15
**最后更新**: 2025-11-15
**负责人**: 开发团队
**审核状态**: 待审核

**文档说明**:
本文档为生产管理系统开发计划的细化版，包含：
- 4个完整开发阶段详细方案
- 数据库表结构设计（38张新表）
- 核心服务实现代码示例
- 前端组件架构设计
- 技术架构选型说明
- 完整的交付清单和里程碑

**更新历史**:
- v4.0 (2025-11-15): 完成4个阶段细化方案编写
```
